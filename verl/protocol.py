...
class DataProto:
    ...
    def to(self, device) -> "DataProto":
        ...
        if self.batch is not None:
            ...
            dev_type = str(dev).split(":")[0]

            if dev_type == "npu":
                new_batch = self.batch.clone(False)
                debug_path = f"/tmp/dataproto_to_{os.getpid()}.log"

                with open(debug_path, "a") as f:
                    f.write(f"\n=== DataProto.to start target={dev} pid={os.getpid()} ===\n")
                    
                env_msg = (
                    f"[DataProto.to][ENV] pid={os.getpid()} "
                    f"ASCEND_RT_VISIBLE_DEVICES={os.environ.get('ASCEND_RT_VISIBLE_DEVICES')} "
                    f"device_count={torch.npu.device_count()} "
                    f"current_device="
                )
                try:
                    env_msg += str(torch.npu.current_device())
                except Exception as e:
                    env_msg += f"ERR:{repr(e)}"

                print(env_msg, flush=True)
                with open(debug_path, "a") as f:
                    f.write(env_msg + "\n")

                for key in self.batch.keys(include_nested=True, leaves_only=True):
                    value = self.batch.get(key)

                    if isinstance(value, torch.Tensor):
                        msg = (
                            f"[DataProto.to] key={key} "
                            f"dtype={value.dtype} shape={tuple(value.shape)} "
                            f"stride={value.stride()} contiguous={value.is_contiguous()} "
                            f"device={value.device} target={dev}"
                        )
                        ...
                        try:
                            src = value.detach().clone().contiguous()
                            moved = src.to(dev, non_blocking=False)
                        except Exception as e:
                            fail_msg = (
                                f"[DataProto.to][FAILED] key={key} "
                                f"dtype={value.dtype} shape={tuple(value.shape)} "
                                f"stride={value.stride()} contiguous={value.is_contiguous()} "
                                f"device={value.device} target={dev} err={repr(e)}"
                            )
                        ...
                        new_batch.set(key, moved)
                    else:
                        new_batch.set(key, value)

                self.batch = new_batch
            else:
                self.batch = self.batch.to(dev, non_blocking=False)
        return self
