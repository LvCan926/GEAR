...
def _maybe_normalize_single_device_local_rank():
    """When Ray exposes a single visible NPU/GPU to the worker, vLLM should use local device 0."""
    for env_key in ("ASCEND_RT_VISIBLE_DEVICES", "CUDA_VISIBLE_DEVICES"):
...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        _maybe_normalize_single_device_local_rank()
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs_with_n1):
            ...
            if enable_graded_system_prompt and do_sample and not is_validate and original_n > 1 and not participate_training:
                # The rollout used expanded prompts with rubric system messages, but PPO
                # should train on the original prompts when participate_training=False.
                # Reuse the original padded tensors directly instead of reconstructing
                # them from raw_prompt_ids; reconstruction can differ in padding/mask
                # details and break exact equality on some tokenizers/devices.
                idx = _repeat_interleave(prompts.batch["input_ids"], original_n)
                attention_mask = _repeat_interleave(prompts.batch["attention_mask"], original_n)
                position_ids = _repeat_interleave(prompts.batch["position_ids"], original_n)
                batch_size = idx.size(0)
