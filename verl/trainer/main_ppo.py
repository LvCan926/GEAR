...
def _build_ray_runtime_env(extra=None):
    env_vars = {
    ...
    passthrough_prefixes = ("ASCEND_", "HCCL_", "GEAR_", "NPU_", "VLLM_")
...
def run_ppo(config) -> None:
    ...
    if not ray.is_initialized():
        ...
        print(f"[Ray] initializing with address={ray_address}")
        ray.init(
            address=ray_address,
            runtime_env=_build_ray_runtime_env(),
    ...
    if OmegaConf.select(config.trainer, "profile_steps") is not None and len(OmegaConf.select(config.trainer, "profile_steps")) > 0:
        ...
        runner = TaskRunner.options(runtime_env=_build_ray_runtime_env({"nsight": nsight_options})).remote()
    else:
...
class TaskRunner:
    def run(self, config):
        ...
        if config.get("val_custom_reward_function"):
            ...
            val_reward_fn = load_reward_manager(val_config, tokenizer, num_examine=1, **config.reward_model.get("reward_kwargs", {}))
        else:
