...
def _is_numeric_scalar_metric(value: Any) -> bool:
    """Return True only for scalar numeric-like values that are safe to aggregate."""
    ...
    if torch.is_tensor(value):
        return value.ndim == 0 and (
            ...
            or value.dtype in {torch.int8, torch.int16, torch.int32, torch.int64, torch.uint8, torch.bool}
        )
    ...
    for data_source, prompt2var2vals in data_src2prompt2var2vals.items():
        for prompt, var2vals in prompt2var2vals.items():
            for var_name, var_vals in var2vals.items():
                if not var_vals or not _is_numeric_scalar_metric(var_vals[0]):
                    continue
