...
def _to_non_tensor_array(values):
    """Preserve structured reward metadata without requiring homogeneous shapes."""
    try:
...
class RayPPOTrainer:
    ...
    def _dump_generations(self, inputs, outputs, scores, reward_extra_infos_dict, dump_path, graded_prompt_info=None):
        ...
        # Add graded system prompt information
        if graded_prompt_info is not None and len(graded_prompt_info) == n:
            # Extract graded system prompt related fields
            rubric_counts = []
            actual_rubric_counts = []
            rubric_ratios = []
            system_prompts = []
            combined_prompt_texts = []
            original_indices = []
            graded_indices = []
            ...
            base_data["combined_prompt_text"] = combined_prompt_texts
        ...
        # If there is graded system prompt information, print additional statistics
        if graded_prompt_info is not None and len(graded_prompt_info) == n:
            ...
            print(f"  - Graded system prompt statistics: {len(graded_prompt_info)} records")
            print(f"  - Total Rubric count distribution: {sorted(unique_rubric_counts)}")
            print(f"  - Actual displayed Rubric count distribution: {sorted(unique_actual_rubric_counts)}")
            print(f"  - Ratio distribution: {sorted(unique_ratios)}")
    ...
    def _dump_validation_metrics(self, metric_dict, dump_path):
        """Append validation summary metrics as JSONL for easy offline inspection."""
        os.makedirs(dump_path, exist_ok=True)
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # Check if graded system prompt feature is enabled, reward_model field needs to be retained during validation
            enable_graded_system_prompt = getattr(self.config.actor_rollout_ref.rollout, 'enable_graded_system_prompt', False)
            validation_reward_models = None
            if enable_graded_system_prompt and "reward_model" in test_batch.non_tensor_batch:
                print(f"[RAY_TRAINER] Validation phase: Graded system prompt feature enabled, passing reward_model info through meta_info")
                
                # Extract reward_model information and put it into meta_info as additional data channel
                reward_models = []
                for i in range(len(test_batch)):
                    reward_model = test_batch.non_tensor_batch['reward_model'][i]
                    reward_models.append(reward_model)
                
                # Store reward_model information in meta_info
                test_gen_meta_info = {
                    "graded_system_prompt_reward_models": reward_models,
                    "enable_graded_system_prompt": True
                }
                validation_reward_models = reward_models
                print(f"[RAY_TRAINER] Validation phase: Successfully extracted {len(reward_models)} reward_models to meta_info")
                
                # reward_model can be popped from non_tensor_batch
                non_tensor_batch_keys_to_pop.append("reward_model")
            else:
                # If graded system prompt feature is not enabled, reward_model field can be popped
            ...
            if validation_reward_models is not None:
                test_batch.non_tensor_batch["reward_model"] = _to_non_tensor_array(validation_reward_models)

            # Add reward_model information to test_gen_batch's meta_info
            test_gen_batch.meta_info.update(test_gen_meta_info)
            
            # Add training step information to meta_info (for step_linear rule)
            ...
            test_batch.batch["attention_mask"] = test_batch.batch["attention_mask"].bool().to(
                dtype=test_batch.batch["input_ids"].dtype
            )
        ...
        if val_data_dir:
            self._dump_generations(
                ...
                graded_prompt_info=None,  # Validation phase usually does not use graded system prompt
            )
        ...
        if val_data_dir:
            self._dump_validation_metrics(metric_dict=metric_dict, dump_path=val_data_dir)
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # Check if graded system prompt feature is enabled, if enabled then retain reward_model field
                enable_graded_system_prompt = getattr(self.config.actor_rollout_ref.rollout, 'enable_graded_system_prompt', False)
                if enable_graded_system_prompt and "reward_model" in batch.non_tensor_batch:
                    print(f"[RAY_TRAINER] Graded system prompt feature enabled, passing reward_model info through meta_info")
                    ...
                    # Store reward_model information in meta_info as backup data channel
                    gen_batch_meta_info = {
                        "graded_system_prompt_reward_models": reward_models,
                        "enable_graded_system_prompt": True
                    }
                    print(f"[RAY_TRAINER] Successfully extracted {len(reward_models)} reward_models to meta_info")
                    
                    # reward_model can be popped from non_tensor_batch since we already pass it through meta_info
                    non_tensor_batch_keys_to_pop.append("reward_model")
                else:
                    # If graded system prompt feature is not enabled, reward_model field can be popped
                ...
                # Add reward_model information to gen_batch's meta_info
                ...
                with marked_timer("step", timing_raw):
                    ...
                    batch.batch["attention_mask"] = batch.batch["attention_mask"].bool().to(
                    ...
                    batch.meta_info["global_token_num"] = torch.sum(batch.batch["attention_mask"].bool(), dim=-1).tolist()
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if reward_extra_infos_dict:
                            batch.non_tensor_batch.update({k: _to_non_tensor_array(v) for k, v in reward_extra_infos_dict.items()})
                    ...
                    # Reference Model Reset Logic
                    ref_reset_freq = self.config.trainer.get("ref_reset_freq", -1)
                    if self.use_reference_policy and not self.ref_in_actor and ref_reset_freq > 0 and self.global_steps % ref_reset_freq == 0:
                        print(f"\n[Step {self.global_steps}] Resetting Reference Model to current Actor Model weights...")
                        try:
                            import tempfile
                            import shutil
                            ...
                            with tempfile.TemporaryDirectory() as temp_dir:
                                actor_state_path = os.path.join(temp_dir, "actor_temp_checkpoint")
                                
                                # Save current actor model state
                                print(f"[Step {self.global_steps}] Saving actor model to temporary checkpoint...")
                                self.actor_rollout_wg.save_checkpoint(
                                    local_path=actor_state_path,
                                    remote_path=None,
                                    global_step=self.global_steps,
                                    max_ckpt_to_keep=1
                                )
                                
                                # Load actor state into reference model
                                print(f"[Step {self.global_steps}] Loading actor weights into reference model...")
                                self.ref_policy_wg.load_checkpoint(
                                    local_path=actor_state_path,
                                    del_local_after_load=True
                                )
                                
                                print(f"[Step {self.global_steps}] Reference Model successfully reset to Actor Model weights.")
                                
                        except Exception as reset_e:
                            print(f"ERROR during reference model reset at step {self.global_steps}: {reset_e}")
                            import traceback
                            traceback.print_exc()
