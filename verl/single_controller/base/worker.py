...
from verl.utils.device import maybe_set_runtime_device
...
class Worker(WorkerHelper):
    ...
    def _configure_before_init(self, register_center_name: str, rank: int):
        ...
        if rank == 0:
            # Force a reachable address in single-node runs to avoid resolving an invalid address again.
            master_addr = "172.16.0.201"
            master_port = str(self._get_free_port())

            rank_zero_info = {
                "MASTER_ADDR": str(os.environ.get("MASTER_ADDR", master_addr)),
                "MASTER_PORT": str(os.environ.get("MASTER_PORT", master_port)),
            }

            print(
                f"[DEBUG rank0] computed master_addr={master_addr}, master_port={master_port}",
                flush=True,
            )
            print(
                f"[DEBUG rank0] before os.environ.update, "
            ...
            print(
                f"[DEBUG rank0] after os.environ.update, "
                f"MASTER_ADDR={os.environ.get('MASTER_ADDR')}, "
                f"MASTER_PORT={os.environ.get('MASTER_PORT')}",
                flush=True,
            )
            ...
            # Wait for rank 0 to create the register center to avoid a race condition.
            ...
            if self.register_center is None:
                raise RuntimeError(
                    f"Failed to get register_center actor {register_center_name} "
    ...
    def _setup_env_cuda_visible_devices(self):
        ...
        ray_local_rank = os.environ.get("RAY_LOCAL_RANK")
        ray_local_world_size = os.environ.get("RAY_LOCAL_WORLD_SIZE")

        if os.environ.get("LOCAL_RANK") is None and ray_local_rank is not None:
            os.environ["LOCAL_RANK"] = ray_local_rank
        if os.environ.get("LOCAL_WORLD_SIZE") is None and ray_local_world_size is not None:
            os.environ["LOCAL_WORLD_SIZE"] = ray_local_world_size
