...
def _normalize_pi_mode(pi_mode: str) -> str:
    normalized = str(pi_mode or "prob").strip().lower()
    if normalized == "binary":
        return "binary"
    if normalized in {"prob", "judge_prob", "normalized_score", "score", "soft"}:
        return "prob"
    raise ValueError(f"Unsupported PLawBench pi_mode={pi_mode!r}; use 'binary' or 'prob'.")
...
def compute_score_batched(
    ...
    final_reward_mode: str = "standard",
    **kwargs: Any,
) -> List[Dict[str, Any]]:
